...
<script>
let seq=0,sessionId=null,sessionKey=null,refreshInFlight=false;const pending=new Map();
...
function bindInput(params){const args=toolArguments(params);if(!args)return;const priorId=sessionId,priorKey=sessionKey;if(typeof args.session_id==='string'&&args.session_id)sessionId=args.session_id;if(typeof args.session_key==='string'&&args.session_key)sessionKey=args.session_key;if((sessionId||sessionKey)&&(sessionId!==priorId||sessionKey!==priorKey))void refresh()}
...
async function refresh(){if(!sessionId&&!sessionKey){document.getElementById('status').textContent='Waiting for task binding…';return}if(refreshInFlight)return;refreshInFlight=true;try{const result=await call('tools/call',{name:'read_advisor_feed',arguments:{session_id:sessionId,session_key:sessionKey,limit:100}});render(result?.structuredContent||result)}catch(e){document.getElementById('status').textContent='Refresh failed'}finally{refreshInFlight=false}}
function bootstrapCompatibility(){const bridge=window.openai;if(!bridge)return false;if(bridge.toolInput)bindInput(bridge.toolInput);if(bridge.toolOutput)render(bridge.toolOutput?.structuredContent||bridge.toolOutput);return Boolean(sessionId||sessionKey)}
async function initializeBridge(){bootstrapCompatibility();try{const initialized=await call('ui/initialize',{protocolVersion:'2025-06-18',appInfo:{name:'Advisor feed',version:'1.0.0'},capabilities:{}});bindInput(initialized?.toolInput||initialized?.context?.toolInput||null)}catch{}if(bootstrapCompatibility())return;let attempts=0;const timer=setInterval(()=>{attempts+=1;if(bootstrapCompatibility()||attempts>=20)clearInterval(timer)},100)}
document.getElementById('refresh').addEventListener('click',refresh);void initializeBridge();setInterval(()=>{if(sessionKey)refresh()},3000);
</script></body></html>`;
