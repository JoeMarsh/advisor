...
<script>
let seq=0,sessionKey=null;const pending=new Map();
...
addEventListener('message',event=>{const m=event.data;if(!m||m.jsonrpc!=='2.0')return;if(m.id&&pending.has(m.id)){const p=pending.get(m.id);pending.delete(m.id);m.error?p.reject(m.error):p.resolve(m.result);return}if(m.method==='ui/notifications/tool-result')render(m.params?.structuredContent||m.params?.result?.structuredContent||m.params?.result)});
const number=n=>new Intl.NumberFormat(undefined,{notation:'compact',maximumFractionDigits:1}).format(Number(n||0));
const pct=n=>Number(n||0).toFixed(1)+'%';
function render(data){if(!data)return;sessionKey=data.session_key||sessionKey;document.getElementById('status').textContent=[data.activity?.status,data.activity?.model,data.activity?.reasoning_effort,'updated '+new Date(data.updated_at).toLocaleTimeString()].filter(Boolean).join(' · ');document.getElementById('cards').innerHTML=[['Main tokens',number(data.usage?.main_total)],['Advisor tokens',number(data.usage?.advisor_total)],['Advisor / Main',pct(data.usage?.advisor_main_ratio)]].map(x=>'<div class="card"><div class="value">'+x[1]+'</div><div class="label">'+x[0]+'</div></div>').join('');const feed=data.feed||[];document.getElementById('feed').innerHTML=feed.length?feed.map(e=>'<div class="entry '+escapeHtml(e.severity)+'"><div class="row"><span class="badge">'+escapeHtml(e.origin)+' · '+escapeHtml(e.severity)+'</span><span class="meta">'+new Date(e.created_at*1000).toLocaleTimeString()+'</span></div><div class="note">'+escapeHtml(e.note)+'</div></div>').join(''):'<div class="empty">No Advisor advice has been emitted for this task yet.</div>'}
function escapeHtml(s){return String(s||'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]))}
async function refresh(){try{const result=await call('tools/call',{name:'read_advisor_feed',arguments:{session_key:sessionKey,limit:100}});render(result?.structuredContent||result)}catch(e){document.getElementById('status').textContent='Refresh failed'}}
