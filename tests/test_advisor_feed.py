...
class AdvisorFeedMcpTests(unittest.TestCase):
    def test_feed_server_exposes_read_only_ui_and_lane_origin(self) -> None:
        ...
        self.assertIn("window.openai", resource["text"])
        self.assertIn("bridge.toolInput", resource["text"])
        self.assertIn("bridge.toolOutput", resource["text"])
        self.assertIn("void initializeBridge()", resource["text"])
        self.assertIn("if(!sessionId&&!sessionKey)", resource["text"])
        self.assertIn("if(refreshInFlight)return", resource["text"])
